...
let pendingCriticalId = null; // consumed when a profile entry finishes reopening its game
...
async function onRetryMove(orig, dest) {
  ...
  const promo = isPromotion(orig, dest) ? "q" : undefined;
  const uci = orig + dest + (promo || "");
  const move = chess.move({ from: orig, to: dest, promotion: promo });
  if (!move) {
    renderBoard();
    return;
  }
  ...
  try {
    ...
    if (!response.ok || result.error) throw new Error(result.error || `Engine error (${response.status})`);
  } catch (error) {
...
async function showRetryHint() {
  ...
  try {
    ...
    const response = await fetch("/api/training/hint?" + q.toString());
    result = await response.json();
    if (!response.ok || result.error) throw new Error(result.error || "Hint unavailable.");
  } catch (error) {
...
async function trainActiveCritical() {
  ...
  puzzleCategory = (critical.facts && critical.facts.primary_category) || "";
...
function startReviewVariation(kind, step = 0, autoplay = false) {
  ...
  const probe = new Chess(critical.fen_before);
...
async function generateReviewExplanations() {
  ...
  for (let index = 0; index < targets.length; index++) {
    ...
    try {
      const response = await fetch(`/api/games/${encodeURIComponent(currentGameId)}/explanations`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          review_side: player,
      ...
      if (token !== explanationGeneration) return;
      if (!response.ok || data.error) throw new Error(apiErrorMessage(data.error, "Explanation failed."));
...
async function loadReviewArtifacts(gameId, side) {
  ...
  try {
    response = await fetch(
      `/api/games/${encodeURIComponent(currentGameId)}/analysis?review_side=${encodeURIComponent(side)}`
    );
    if (!response.ok) throw new Error("analysis artifact unavailable");
    engineReview = await response.json();
  } catch (_) {
  ...
  try {
    const expResponse = await fetch(
      `/api/games/${encodeURIComponent(currentGameId)}/explanations?review_side=${encodeURIComponent(side)}`
    );
    if (expResponse.ok) explanationArtifact = await expResponse.json();
  } catch (_) {}
  const ready = ((explanationArtifact && explanationArtifact.positions) || []).length;
  const total = criticalPositions.length;
  setWorkflowState(
...
async function loadAll() {
  ...
  try {
    ...
    defaultReviewSide = cfg.default_review_side || "auto";
    boardOrientationPreference = cfg.board_orientation || "review";
    analysisPreset = cfg.analysis_preset || "balanced";
    explanationProvider = cfg.explanation_provider || "auto";
    explanationLanguage = cfg.explanation_language || "zh-CN";
    showThreatsByDefault = cfg.show_threat_arrows === true;
    threatArrowOn = showThreatsByDefault;
    $("threat-toggle").checked = threatArrowOn;
    $("paste-side").value = defaultReviewSide;
  } catch (_) {}
...
async function openGame(pgn, side, gameId = null) {
  ...
  try {
    const endpoint = gameId
      ? `/api/games/${encodeURIComponent(gameId)}/analyze`
      : "/api/analyze";
    const requestBody = gameId
      ? { review_side: side || "auto" }
      : { pgn, player: side || "auto" };
    st = await fetch(endpoint, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function renderHistory(games, mode, who) {
  ...
  for (const g of games) {
    ...
    if (disabled) {
      ...
      li.addEventListener("click", () => openGame(g.pgn, side, mode === "normal" ? g.game_id : null));
    }
    ol.appendChild(li);
  }
}
...
async function deleteHistoryGame(game) {
  ...
  try {
    const response = await fetch(`/api/games/${encodeURIComponent(game.game_id)}`, { method: "DELETE" });
    const data = await response.json();
...
async function loadInsights() {
  ...
  let data;
  try {
    data = await fetch(`/api/profile?days=${insightsDays}`).then((r) => r.json());
  } catch (_) {
...
async function openProfilePosition(weakness) {
  ...
  pendingCriticalId = target.critical_id;
  if (puzzleMode) await setPuzzleMode(false);
  openGame(row.pgn, target.reviewed_side, target.game_id);
}
...
async function trainProfileWeakness(weakness) {
  ...
  puzzleSource = "your_games";
  puzzleCategory = weakness.category || "";
  lsSet(PZ_SOURCE_KEY, puzzleSource);
  lsSet(PZ_CATEGORY_KEY, puzzleCategory);
  await setPuzzleMode(true, {
    puzzle: {
      ...
      critical_id: target.critical_id,
    },
  });
}
...
async function clearEngineCache() {
  ...
  try {
    const response = await fetch("/api/data/engine-cache/clear", { method: "POST" });
    const data = await response.json();
    if (!response.ok || data.error) throw new Error(data.error || "Cache cleanup failed.");
...
async function saveSettings(e) {
  ...
  defaultReviewSide = (res.settings && res.settings.default_review_side) || "auto";
  boardOrientationPreference = (res.settings && res.settings.board_orientation) || "review";
  analysisPreset = (res.settings && res.settings.analysis_preset) || "balanced";
  explanationProvider = (res.settings && res.settings.explanation_provider) || "auto";
  explanationLanguage = (res.settings && res.settings.explanation_language) || "zh-CN";
  showThreatsByDefault = !!(res.settings && res.settings.show_threat_arrows);
  $("paste-side").value = defaultReviewSide;
  threatArrowOn = showThreatsByDefault;
  $("threat-toggle").checked = threatArrowOn;
  if (timeline.length) {
...
async function startPasteAnalysis(pgn, side, username, sourceType = pasteSourceType) {
  ...
  setWorkflowState("importing", "Importing PGN", "Normalizing headers and replaying legal moves.");
  submit.disabled = true;
  let response, data;
  try {
    response = await fetch("/api/games/import", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        pgn,
    ...
    setWorkflowState("failed", "Import failed", "Could not reach the local import service.");
  ...
  if (!response.ok || data.error) {
    ...
    setWorkflowState("failed", "Import failed", status.textContent);
    return;
  }
  ...
  if (unresolved.length && side === "auto") {
    ...
    $("paste-side").focus();
    setWorkflowState("ready_to_analyze", "Game imported", "Choose a review side to continue.");
    return;
  }
  ...
  if (imported.length > 1) {
    ...
    openBatch(normalized, side, username);
    return;
  }

  const game = imported[0];
  const selected = game.review_side || side;
  setWorkflowState("ready_to_analyze", "Game ready to analyze", `Reviewing ${selected}.`);
  status.classList.add("import-success");
  ...
  openGame(game.pgn, selected, game.game_id);
}
...
async function loadPuzzleCategories() {
  const select = $("pz-category");
  if (!select) return;
  let categories = [];
  try {
    const result = await fetch("/api/puzzle/categories").then((response) => response.json());
...
function init() {
  ...
  $("prev-mistake").addEventListener("click", () => {
    const index = criticalPositions.findIndex((item) => item.critical_id === activeCriticalId);
    if (criticalPositions.length && index > 0) selectCritical(criticalPositions[index - 1].critical_id);
    else if (!criticalPositions.length && currentMistake > 0) selectMistake(currentMistake - 1);
  });
  $("next-mistake").addEventListener(
    "click",
    () => {
      const index = criticalPositions.findIndex((item) => item.critical_id === activeCriticalId);
      if (criticalPositions.length && index >= 0 && index < criticalPositions.length - 1) {
        selectCritical(criticalPositions[index + 1].critical_id);
      } else if (!criticalPositions.length && currentMistake < mistakes.length - 1) {
        selectMistake(currentMistake + 1);
      }
    }
  );
  ...
  $("critical-prev").addEventListener("click", () => {
    const index = criticalPositions.findIndex((item) => item.critical_id === activeCriticalId);
    if (index > 0) selectCritical(criticalPositions[index - 1].critical_id);
  });
  $("critical-next").addEventListener("click", () => {
    const index = criticalPositions.findIndex((item) => item.critical_id === activeCriticalId);
    if (index >= 0 && index < criticalPositions.length - 1) selectCritical(criticalPositions[index + 1].critical_id);
  });
  ...
  $("pz-category").addEventListener("change", (e) => {
    puzzleCategory = e.target.value || "";
    lsSet(PZ_CATEGORY_KEY, puzzleCategory);
