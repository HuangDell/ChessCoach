...
**状态：** P0 已有离线回放与 SDK/fake 接线实现；P1–P3 方法比较及 live 研究结果待完成

**近期安排（2026-09-13）：** 先完成原生工具编排对照，再增加教学知识检索与按需 RAG，见
[工具编排与 RAG 路线](tool-orchestration-rag-roadmap.md)。本文件保留完整研究范围。
...
P0 的开发任务、状态回放和 SDK/fake 接线已落入 [研究设施](../../tests/evals/orchestration/README.md)，
不代表真实模型上的方法收益。live 成本测量、正式对照、作者代码核验与研究结果仍待完成。
下一步按[近期路线](tool-orchestration-rag-roadmap.md)复核 P0、建立原生工具 live 基线，
再开展执行状态对照和教学知识 RAG；该近期切片不等同于完成本文件 P2 的全部研究验收。
