...
因此首次分析比仅筛失误更慢；进度显示验证阶段，已完成结果使用版本化缓存。
扫描、正面验证和关键局面深度分析各阶段内按独立局面并行，阶段之间保留依赖顺序。
并发数复用 `CHESS_ENGINE_POOL_SIZE`（默认 2，设为 1 即串行），多个分析任务共享同一进程池。
进度按已完成局面计数，结果仍按棋谱顺序汇总。CPU 线程预算约为 pool size ×
`CHESS_ENGINE_THREADS`，哈希内存约为 pool size × `CHESS_ENGINE_HASH_MB`；增加并发不保证线性加速。

有限搜索、仅检查新暴露棋子的直接吃子和保守的物质投入条件可能漏掉复杂妙手；没有获奖不代表没有精彩之处。
...
桌面三栏会锁定页面滚动，导航栏和 Analysis 栏分别独立滚动，棋盘保持固定；导航栏内容超出时，
摘要、胜率图和列表一起滚动，不再嵌套列表滚动区。导航默认
