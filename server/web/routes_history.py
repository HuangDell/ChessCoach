...
@router.get("/profile")
def get_local_profile(days: int = 30) -> dict:
    """Deterministic local profile, including weaknesses, training outcomes, and coach summary."""
    try:
        return history.insights(days if days > 0 else None)
    except Exception as exc:  # noqa: BLE001 - a damaged personal file must not break the board
        return {"games": 0, "error": str(exc)}
...
def post_clear_engine_cache() -> JSONResponse:
    """Delete recomputable analysis caches only; permanent artifacts and history remain intact."""
    try:
        return JSONResponse({"ok": True, **clear_engine_caches()})
    except OSError as exc:
        return JSONResponse({"error": f"Could not clear Engine cache: {exc}"}, status_code=500)
